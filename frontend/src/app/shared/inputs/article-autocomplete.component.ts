    ...
    <mat-form-field class="w-full">
      ...
      <mat-autocomplete requireSelection #auto="matAutocomplete" [displayWith]="displayFn">
        @for (option of options(); track option.id; let i = $index) {
          ...
          @if (i < options().length - 1) { <mat-divider /> }
        }
      </mat-autocomplete>
    </mat-form-field>
  `,
...
export class ArticleAutocompleteComponent {
  ...
  readonly options = signal<Article[]>([]);

  async load() {
    if (!this.all.length) {
      this.all = await this.serverManager.activeServer()!.api.article.list({ categories: this.includeCategories } as any);
    }
    this.filter();
  }
